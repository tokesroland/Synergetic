...
const CalendarView = {
  ...
  computed: {
    ...
    title() {
      ...
      if (this.view === "week") {
        const mon = this.getMonday(this.date),
          sun = new Date(mon);
    ...
    monthCells() {
      ...
      for (let i = 0; i < total; i++) {
        ...
        if (i < startOfs) {
          ...
          isOther = true;
        } else cellDate = new Date(year, month, i - startOfs + 1);
        const de = this.entriesForDate(cellDate);
        cells.push({
          date: cellDate,
    ...
    agendaTodos() {
      return this.todoFor(this.selected);
    },
    agendaEvents() {
    ...
    weekDays() {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        days.push({
          ...
          entries: this.entriesForDate(d),
        });
      }
      return days;
    },
    dayTimedEntries() {
      return this.timedFor(this.date).sort(
        (a, b) =>
          new Date(a.start_datetime?.replace(" ", "T") || 0) -
          new Date(b.start_datetime?.replace(" ", "T") || 0),
      );
    ...
    dayTodos() {
      return this.todoFor(this.date);
    },
    dayAlldayEntries() {
      return this.alldayFor(this.date);
    },
    isDayToday() {
      return this.isToday(this.date);
    },
    hours24() {
  ...
  async mounted() {
    ...
    this.nowInterval = setInterval(() => this.$forceUpdate(), 60000);
  },
  beforeUnmount() {
  ...
  methods: {
    async loadEntries() {
      const d = await ApiService.getCalendarEntries();
      if (d) this.entries = d;
    },
    entriesForDate(date) {
    ...
    isAllDay(e) {
      return e.is_all_day == 1;
    },
    col(type) {
      return this.colors[type] || this.colors.todo;
    },
    todoRole(entry, date) {
      const t = new Date(date);
      t.setHours(0, 0, 0, 0);
    ...
    todoMeta(entry) {
      ...
      } else if (entry.start_datetime) m = "Tervezett kezdés napja";
      return m;
    },
    prev() {
      if (this.view === "month")
    ...
    startDividerDrag(e) {
      ...
      const onMove = (ev) => {
        ...
        this.gridFlex = Math.max(
          20,
          Math.min(80, ((ev.clientY - r.top) / r.height) * 100),
        );
      };
      const onUp = () => {
