...
const GraphView = {
  ...
  computed: {
    ...
    visibleNodes() {
      return Store.nodes.filter((n) => Store.filters[n.type]);
    },
    colors() {
      return Store.colors;
    },
  ...
  methods: {
    ...
    getNodeAt(sx, sy) {
      const w = this.screenToWorld(sx, sy),
        r = this.NODE_RADIUS + this.HOVER_TOLERANCE;
      return (
        this.visibleNodes.find(
          (n) => (n.x - w.x) ** 2 + (n.y - w.y) ** 2 <= r * r,
        ) || null
      );
    },
    getLinkAt(sx, sy) {
      ...
      for (const node of this.visibleNodes) {
        ...
        for (const linkId of node.links) {
          ...
          if (!target) continue;
          if (this.distToSegment(w, node, target) < 8)
    ...
    showZoomBadge() {
      this.zoomBadgeVisible = true;
      clearTimeout(this.zoomBadgeTimeout);
    ...
    onWheel(e) {
      ...
      const oldZoom = this.camera.zoom,
        delta = e.deltaY < 0 ? this.camera.zoomStep : -this.camera.zoomStep;
      const newZoom = Math.min(
        this.camera.maxZoom,
        Math.max(this.camera.minZoom, oldZoom + delta * oldZoom),
      );
    ...
    onMouseDown(e) {
      ...
      if (this.isLinkingMode) {
        const t = this.getNodeAt(sx, sy);
        if (t && t !== this.activeNodeForMenu) this.finishLink(t);
        return;
      }
      if (this.isUnlinkingMode) {
        const l = this.getLinkAt(sx, sy);
        if (l) this.finishUnlink(l);
        return;
      }
      ...
      if (node) {
        ...
        this.dragOriginalPos = { x: node.x, y: node.y };
    ...
    onMouseMove(e) {
      ...
      if (this.isPanning) {
        this.camera.x = this.panCameraStart.x + (e.clientX - this.panStart.x);
        this.camera.y = this.panCameraStart.y + (e.clientY - this.panStart.y);
        this.draw();
        return;
      }
      if (this.isDragging && this.draggedNode) {
        ...
        if (sidebar) {
          ...
          this.isDraggingToGroup = e.clientX < sr.right + 40;
          if (this.isDraggingToGroup && !Store.sidebarOpen)
    ...
    onMouseUp(e) {
      // Clear drop targets
      document
        .querySelectorAll(".group-item.drop-target")
        .forEach((gi) => gi.classList.remove("drop-target"));
      if (this.isPanning) {
              ...
              if (data && !data.error) {
                ...
                if (targetGroupId != currentGroupId) {
                  ...
                  Store.nodes.forEach((n) => {
                    if (n.links)
                      n.links = n.links.filter((lid) => lid !== entryId);
                  });
                }
                Store.loadGroups();
                this.draw();
    ...
    onDblClick(e) {
      ...
      if (node) this.$router.push({ name: "details", params: { id: node.id } });
    },
    onRightClick(e) {
      e.preventDefault();
      const rect = this.canvas.getBoundingClientRect();
      const node = this.getNodeAt(e.clientX - rect.left, e.clientY - rect.top);
      if (node) {
        this.activeNodeForMenu = node;
        Store.selectedId = node.id;
        this.quickMenuPos = { x: e.clientX, y: e.clientY };
    ...
    getGroupUnderCursor(cx, cy) {
      ...
      for (const item of items) {
        ...
        if (cx >= r.left && cx <= r.right && cy >= r.top && cy <= r.bottom)
          return parseInt(item.dataset.group);
      }
      return null;
    },
    closeMenus() {
    ...
    quickAction(action) {
      const node = this.activeNodeForMenu;
      if (!node) return;
      if (action === "link") {
        ...
        this.$router.push({ name: "details", params: { id: node.id } });
        this.closeMenus();
      } else if (action === "category") {
        this.openSubMenu("category");
      } else if (action === "tag") {
        this.openSubMenu("tag");
      } else if (action === "delete") {
        if (confirm(`Biztosan törlöd: "${node.title}"?`)) {
          ApiService.deleteEntry(node.id).then(() => Store.loadCurrentGroup());
        }
        this.closeMenus();
      }
    },
    async openSubMenu(type) {
      this.subMenuAction = type;
      this.subSearch = "";
      if (type === "category") {
        await Store.loadCategories();
        this.subItems = Store.categories;
      } else if (type === "tag") {
        await Store.loadTags();
        this.subItems = Store.tags;
      }
    ...
    async selectSubItem(item) {
      const node = this.activeNodeForMenu;
      if (!node) return;
      if (this.subMenuAction === "category")
        await ApiService.assignCategory(node.id, item.id);
      else if (this.subMenuAction === "tag")
        await ApiService.assignTag(node.id, item.id);
      this.closeMenus();
      Store.loadCurrentGroup();
    },
    async createSubItem() {
      const name = this.subSearch.trim();
      if (!name) return;
      if (this.subMenuAction === "category") {
    ...
    async finishLink(target) {
      await ApiService.createLink(this.activeNodeForMenu.id, target.id);
      this.isLinkingMode = false;
      this.canvas.style.cursor = "default";
      Store.loadCurrentGroup();
    },
    async finishUnlink(link) {
      await ApiService.deleteLink(link.source.id, link.target.id);
      this.isUnlinkingMode = false;
      this.canvas.style.cursor = "default";
      Store.loadCurrentGroup();
    },
    draw() {
      ...
      nodes.forEach((node) => {
        const color = this.colors[node.type] || "#fff",
          isActive = node === this.draggedNode || node === this.hoveredNode,
          isSelected = node.id === Store.selectedId;
        ctx.beginPath();
        ctx.arc(node.x, node.y, this.NODE_RADIUS, 0, Math.PI * 2);
        ctx.shadowBlur = isActive || isSelected ? 24 : 8;
